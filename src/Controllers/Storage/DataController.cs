...
namespace Altinn.Platform.Storage.Controllers
{
    ...
    public class DataController : ControllerBase
    {
        ...
        public async Task<ActionResult> Get(int instanceOwnerPartyId, Guid instanceGuid, Guid dataGuid)
        {
            ...
            {
                Stream dataStream = await _blobRepository.ReadBlob(instance.Org, storageFileName, null);
        ...
        public async Task<ActionResult<DataElementList>> GetMany(int instanceOwnerPartyId, Guid instanceGuid)
        {
            ...
            List<DataElement> dataElements = await ((LocalTest.Services.Storage.Implementation.DataRepository)_dataRepository).ReadAll(instanceGuid);
            ...
            [FromQuery(Name = "generatedFromTask")] string generatedFromTask = null)
        {
            ...
            (long length, DateTimeOffset blobTimestamp) = await _blobRepository.WriteBlob(instance.Org, theStream, newData.BlobStoragePath, null);
            newData.Size = length;
            if (length == 0)
            {
                await _blobRepository.DeleteBlob(instance.Org, newData.BlobStoragePath, null);
            ...
            await _dataService.StartFileScan(instance, dataTypeDefinition, dataElement, blobTimestamp, null, CancellationToken.None);
            ...
            [FromQuery(Name = "generatedFromTask")] string generatedFromTask = null)
        {
            ...
            (long blobSize, DateTimeOffset blobTimestamp) = await _blobRepository.WriteBlob(instance.Org, theStream, blobStoragePathName, null);
        ...
        {
            (Instance instance, long internalId) = await _instanceRepository.GetOne(instanceGuid, true, CancellationToken.None);
        ...
        private async Task<ActionResult<DataElement>> DeleteImmediately(Instance instance, DataElement dataElement)
        {
            ...
            await _blobRepository.DeleteBlob(instance.Org, storageFileName, null);
