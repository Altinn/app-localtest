...
public class DataLockController : ControllerBase
{
    ...
    public async Task<ActionResult<DataElement>> Unlock(int instanceOwnerPartyId, Guid instanceGuid, Guid dataGuid)
    {
        ...
        bool authorized = await _authorizationHelper.AuthorizeAnyOfInstanceActions(HttpContext.User, instance, new List<string>() { "write", "unlock" }, instance.Process.CurrentTask.ElementId);
        if (!authorized)
        {
            return Forbid();
        }
